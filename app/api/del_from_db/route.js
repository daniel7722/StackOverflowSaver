...
export async function DELETE (req) {
  try {
    const { q_id, a_id, username } = req.body;

    console.log('Received delete request for:', { q_id, a_id, username }); // Log received data

    // Start a transaction
    await conn.query('BEGIN');

    // Delete from savedanswers table
    await conn.query('DELETE FROM public.savedanswers WHERE username = $1 AND q_id = $2 AND a_id = $3', [username, q_id, a_id]);
    ...
    console.log('Deletion successful for:', { q_id, a_id, username }); // Log on successful deletion
